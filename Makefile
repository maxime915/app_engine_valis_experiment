...
descriptor: pyproject.toml
	python3 -c 'import yaml;f=open("descriptor.yaml");y=yaml.safe_load(f);y["configuration"]["image"]["file"] = "/app_engine_valis_exp-$(VERSION).tar"; print(yaml.dump(y, default_flow_style=False, sort_keys=False))' > $(tmp_descriptor)
